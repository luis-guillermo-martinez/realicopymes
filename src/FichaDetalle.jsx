...
if (cargando) {
return (<div className="min-h-screen bg-crema flex items-center justify-center"><p className="font-body text-navy text-xl">Cargando ficha...</p></div>)
}
if (error || !negocio) {
return (
...
// GALERÍA: solo fotos subidas (sin portada). Destacado=3, Patrocinado=5
...
<button onClick={() => navigate('/')} className="cursor-pointer flex items-center"><img src="/logo.png" alt="MiPin" className="h-10 md:h-12 w-auto" /></button>
...
{(esDestacado || esPatrocinado) && (<span className={`font-label px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${colors.badge}`}>{plan}</span>)}
<span className="font-label text-dorado-claro text-sm uppercase tracking-wide">{negocio.tipo}</span>
{!esGratuito && negocio.categoria && (<span className="font-label text-dorado-claro text-sm uppercase tracking-wide">{negocio.categoria}</span>)}
...
{esGratuito && negocio.direccion && (<p className="font-body text-crema/80 mt-4 flex items-center gap-2 justify-center md:justify-start"><span>📍</span> {negocio.direccion}</p>)}
</div>
</div>
</div>
</header>
...
{tieneWhatsApp && negocio.whatsapp && (<a href={`https://wa.me/${negocio.whatsapp}`} target="_blank" rel="noopener noreferrer" className="block w-full text-center bg-oliva text-white py-3 rounded-lg font-body font-bold hover:bg-oliva-dark transition">Contactar por WhatsApp</a>)}
</div>
)}
{negocio.email && (
<div>
...
<p className="font-body text-navy text-base font-semibold flex items-start gap-2"><span className="text-dorado">📍</span> {negocio.direccion}</p>
</div>
)}
{getMapsEmbedUrl() && (
...
{getMapsUrl() && (<a href={getMapsUrl()} target="_blank" rel="noopener noreferrer" className="block bg-dorado text-navy py-3 rounded-lg font-body font-bold text-center hover:bg-dorado-claro transition">📍 Ver ubicación en Google Maps</a>)}
{tieneComoLlegar && getMapsUrl() && (<a href={getMapsUrl()} target="_blank" rel="noopener noreferrer" className="block w-full text-center bg-navy text-crema py-3 rounded-lg font-body font-bold hover:bg-navy-dark transition mt-3">🧭 Cómo llegar</a>)}
...
{redes.instagram && (<a href={`https://instagram.com/${redes.instagram.replace('@', '')}`} target="_blank" rel="noopener noreferrer" className="flex-1 bg-gradient-to-r from-purple-500 to-pink-500 text-white py-2 rounded-lg font-body font-bold text-sm text-center hover:opacity-90 transition">Instagram</a>)}
{redes.facebook && (<a href={redes.facebook.startsWith('http') ? redes.facebook : `https://facebook.com/${redes.facebook}`} target="_blank" rel="noopener noreferrer" className="flex-1 bg-blue-600 text-white py-2 rounded-lg font-body font-bold text-sm text-center hover:bg-blue-700 transition">Facebook</a>)}
</div>
</div>
)}
</div>
...
<div className="container mx-auto px-4 text-center"><p className="font-body text-sm text-crema/60">© 2026 MiPin. A un pin de distancia.</p></div>
</footer>
</div>
)
}
